...
Sorting is a classic problem in computer science. In this program, you'll implement a sorting algorithm called **selection sort**. This sort operates **in-place**, which means that it moves elements around within the input list, rather than creating a new list that's sorted.

The basic idea behind selection sort is to notice that in the final sorted list:
