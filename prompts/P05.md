...
*Note:* The intended solution to this program does *not* use `if` statements - which we'll cover next lecture. A solution using `if`s can pass the tests and receive credit, I encourage you to solve it using only boolean operators instead, even if you know how to use conditionals.
